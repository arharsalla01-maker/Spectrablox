...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const hasPermission = member.roles.cache.some(role => 
      ['helper', 'mod', 'admin', 'owner'].some(name => role.name.toLowerCase().includes(name))
    );

    if (!hasPermission) {
      return interaction.reply({ 
        content: '❌ No tienes permiso para reclamar este ticket. Solo helpers, mods, admins y owners pueden reclamar.', 
        ephemeral: true 
      });
    }
